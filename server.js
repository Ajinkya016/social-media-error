...
var accessTokenSecret = "DoubleSocialMedia";
...
http.listen(port, function () {
	...
	mongoClient.connect("mongodb://localhost:27017", function (error, client) {
		var database = client.db("my_double_social_network");
		...
		app.post("/uploadCoverPhoto", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					result.json({
						"status": "error",
						"message": "User has been logged out. pls login again."
					});
				} else {
					...
					if (request.files.coverPhoto.size > 0 && request.files.coverPhoto.type.includes("image")) {
						...
						result.json({
							"status": "error",
							"message": "Please select a valid cover photo",
						});
					}
				}
			});
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					if (request.files.video.size > 0 && request.files.video.type.includes("video")) {
						...
						// Read the file
						fileSystem.readFile(request.files.video.path, function (error, data) {
							//
							// Write the file
							fileSystem.writeFile(video, data, function (error) {
								//
							});

							// Delete the file
							fileSystem.unlink(request.files.video.path, function (error) {
								//                                                         
							});
						});
					}
